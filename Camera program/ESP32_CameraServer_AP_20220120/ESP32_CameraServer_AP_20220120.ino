...
void SocketServer_Test(void) {
  ...
  //If available for current client
  if (client) {
    WA_en = true;
    ED_client = true;
    ...
    while (client.connected()) {
      ...
      //Regularly detect connected devices
      if (millis() - Test_time > 1000) {
        Test_time = millis();
        //Serial2.println(WiFi.softAPgetStationNum());

        //If the number of connected devices is "0", send a stop command to the car model
...
void FactoryTest(void) {
  ...
  //The number of connected devices is not "0" and the LED indicator light is always on
