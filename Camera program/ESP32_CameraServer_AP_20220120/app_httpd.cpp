...
//Frame transmission (live video) APP
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    while (true) {
        detected = false;
        face_id = 0;
        fb = esp_camera_fb_get(); //Get an image frame
        ...
        if (res == ESP_OK)
            res = httpd_resp_send_chunk(req, (const char *)_jpg_buf, _jpg_buf_len); //Original send
...
// //Picture frame streaming (live video), test
...
//         detected = false;
//         face_id = 0;
//         fb = esp_camera_fb_get(); //Get a frame of image
//         if (!fb) {
//             Serial.println("Camera capture failed");
//             res = ESP_FAIL;
//         } else {
...
void startCameraServer() {
    ...
    config.server_port += 1; //Video streaming port
    config.ctrl_port += 1;
