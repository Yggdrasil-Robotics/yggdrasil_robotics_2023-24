...
void CameraWebServer_AP::CameraWebServer_AP_Init(void) {
  ...
  config.xclk_freq_hz = 20000000;//Default: 10000000
  ...
  // s->set_framesize(s, FRAMESIZE_SXGA); //Byte length sample value:60000                #9 (High quality)  1280x1024
  s->set_framesize(s, FRAMESIZE_SVGA); //Byte length sample value:40000                   #7 (Medium quality)  800x600
  // s->set_framesize(s, FRAMESIZE_QVGA); //Byte length sample value:10000                #4 (Low quality)  320x240

#if defined(CAMERA_MODEL_M5STACK_WIDE)
  s->set_vflip(s, 0);
  s->set_hmirror(s, 1);
#endif
  s->set_vflip(s, 0);   //Picture orientation settings (up and down)
